...
export default function BordeauxEnPage() {
  return (
    <div className="min-h-screen bg-white pt-32 pb-24">
      <div className="max-w-4xl mx-auto px-6">
        ...
        <h1 className="text-4xl md:text-5xl font-bold text-gray-900 leading-tight mb-6">
          ...
          <span className="text-[#FF5A1F]">in Bordeaux, France</span>
        </h1>
        <p className="text-lg text-gray-500 max-w-2xl mb-10 leading-relaxed">
          NBN IA serves businesses in Bordeaux and the surrounding Gironde area. We build autonomous AI systems with Claude AI agents, custom web apps, and digital automations that run 24/7 without human intervention.
        </p>
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-20">
          {[
            ...
            <Link key={s.title} href={s.href} className="p-6 border border-gray-200 rounded-2xl hover:border-[#FF5A1F] transition-colors group">
              <h3 className="font-bold text-gray-900 mb-2 group-hover:text-[#FF5A1F] transition-colors">{s.title}</h3>
              <p className="text-sm text-gray-500 mb-4">{s.desc}</p>
              <span className="text-xs text-[#FF5A1F] font-semibold flex items-center gap-1">Learn more <ArrowRight size={12} /></span>
            </Link>
          ))}
        </div>
        <div className="bg-gray-50 rounded-2xl p-10 mb-20">
          <h2 className="text-2xl font-bold text-gray-900 mb-6">Why NBN IA in Bordeaux?</h2>
          <ul className="space-y-4">
            {[
              "Local presence in Bordeaux — we meet in person or work 100% remote",
              ...
              <li key={f} className="flex items-start gap-3">
                <Check size={18} className="text-[#FF5A1F] flex-shrink-0 mt-0.5" />
        ...
        <div className="bg-gray-900 rounded-3xl p-12 text-center">
          <h2 className="text-white font-bold text-2xl mb-3">Ready to automate in Bordeaux?</h2>
