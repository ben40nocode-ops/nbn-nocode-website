...
export const articlesEN: ArticleEN[] = [
  {
    ...
    keywords: ["AI back-office travel agency", "travel agency automation", "AI PDF invoice extraction", "Claude API document extraction", "Next.js Supabase", "project handover", "custom internal tool", "Nortrip Toolbox"],
    readingTime: "7 min",
    content: "<p>Over two months, I designed and built the <strong>Nortrip Toolbox</strong> for Nortrip, a bespoke travel agency specialising in Far North trips. The goal: replace a stack of fragile Zapier automations and manual copy-paste with a real internal tool, powered by AI.</p><p>Here is the case study — from prototype to full handover — for those interested in the technical detail.</p><h2>The problem: an agency's invisible work</h2><p>Like many agencies, all the back-office work relied on manual steps and fragile automations: supplier invoices to sort, payment proofs to reconcile, new trips to publish, website photos and copy to update. Each task took time, and every broken Zapier automation was a source of errors.</p><h2>The stack</h2><ul><li><strong>Next.js (App Router) + TypeScript</strong>, deployed on Vercel</li><li><strong>Supabase (Postgres)</strong> as a single database, shared between the Toolbox and the public website</li><li><strong>Claude API (Anthropic)</strong> for all document extraction</li><li><strong>Microsoft Graph / OneDrive</strong> for archiving, with MSAL browser-side authentication</li></ul><h2>The interesting parts</h2><h3>Supplier invoice extraction</h3><p>The PDF goes to Claude with a prompt that enforces <strong>strict JSON</strong>: amount, hotel, due date, file reference. The model returns structured data — no text to parse by hand. The invoice is then automatically filed in the right place.</p><h3>Duplicate detection</h3><p>Before every upload to OneDrive, a check via Graph (<em>fileExists</em>): on a collision, the app offers Replace or Rename -1 instead of silently overwriting a file. A genuine business safeguard — you never lose data by accident.</p><h3>Payment reconciliation</h3><p>A payment proof (Keewe) arrives by email. A webhook POSTs the PDF to a Next API route, then Claude extraction, an upsert into Supabase and OneDrive archiving. All with <strong>fallbacks</strong> (for example the reception date if the model misses a field) so no transaction is ever lost.</p><h3>Code-free publishing and editing</h3><p>A new trip PDF is published to the website with a single drag-and-drop, structured automatically. And website photos and copy can be edited without touching a single line of code.</p><p>On the security side, <strong>two-level authentication</strong> (admin / toolbox) and delegated MSAL to write into the team's OneDrive.</p><p><strong>Result:</strong> 3 Zapier automations replaced, time given back on repetitive tasks, and zero re-entry.</p><h2>The real point: the handover</h2><p>But what I'm most proud of isn't the tech. It's what I did afterwards.</p><p>An internal tool that depends solely on its creator is a <strong>liability, not an asset</strong>. So I migrated the entire ecosystem to the client's own accounts — in a single day, with no downtime:</p><ul><li><strong>GitHub</strong>: repo transfer (redirects handle the history)</li><li><strong>Supabase</strong>: project transfer — URL and keys unchanged, so no variables to repoint</li><li><strong>Vercel</strong>: re-import (direct transfer to a free account doesn't exist), preserving the webhook endpoint</li></ul><p>A detail I like: on a free Vercel plan with a private repo, only the owner's commits deploy. So I aligned the git commit identity with the client's, and the git push to deploy flow keeps working, for free, without me.</p><p>The result: a Nortrip team member now edits the code and ships to production autonomously. My access is no longer a single point of failure.</p><h2>My conviction</h2><p>Too many projects create dependency: the one person who understands the tool. I wanted the opposite. Delivering an AI tool is good. Delivering the tool <strong>plus the keys, the architecture and the autonomy</strong> that go with it — that's the job done.</p><p>AI is only useful if it gives time back to humans — and a tool is only truly successful the day its creator can step away.</p><p>Do you have a back-office running on manual steps or fragile automations? I offer a <strong><a href='https://calendly.com/ben40nocode/1h'>free one-hour audit</a></strong> to identify what can be automated — and deliver a tool you keep the keys to.</p><p><em>Benjamin Bertigny — Founder NBN IA. I build custom AI systems for SMBs. Based in Bordeaux, serving all of France.</em></p>"
  },
  {
    slug: "vibe-hangover-2026-ai-quality-crisis",
    title: "Vibe Hangover: The Morning After AI's Euphoria",
    description: "92% of developers use AI tools daily, but trust is collapsing. What the 2026 \"vibe hangover\" actually means for SMBs and builders.",
    date: "2026-06-04",
    author: "Benjamin Bertigny",
    category: "Build in Public",
    keywords: ["vibe coding 2026", "vibe hangover AI", "AI code quality", "engineer the vibe", "citizen developer SMB", "SaaSpocalypse"],
    readingTime: "5 min",
    content: "<p>In early 2025, Andrej Karpathy coined \"vibe coding.\" By late 2025, everyone was doing it. By 2026, the adoption debate is over — 92% of US developers use AI tools daily, 46% of new code is AI-generated. The market is worth $4.7 billion and projected to hit $12.3 billion in 2027.</p><p>The problem is what you find when the euphoria wears off.</p><h2>Trust is collapsing while usage climbs</h2><p>That's the real signal of 2026. Developer confidence in AI tools has dropped from 77% in 2023 to 60% in 2026. Only 33% trust the accuracy of AI-generated code — down from 43% in 2024. The industry has become dependent on something it doesn't trust.</p><p>The pattern is well-documented: an indie builder ships a fully vibe-coded SaaS, it works, users sign up — and weeks later, it collapses. Exposed API keys, bypassable subscriptions, architecture that buckles under load. Organisations that rushed to replace engineers with prompts quickly ran into security, maintenance, and architectural integrity problems they hadn't seen coming.</p><p>That's the vibe hangover.</p><h2>The shift: \"engineer the vibe\"</h2><p>The phrase circulating in mature tech circles in 2026: you don't \"vibe code\" anymore — you \"engineer the vibe.\" The distinction matters.</p><p>In practice, it means three things:</p><ul><li><strong>Inject your architecture standards directly into the AI's context</strong> — rule files, CLAUDE.md, custom skills, house patterns. The agent follows your conventions, not its defaults.</li><li><strong>Never ship code you don't understand.</strong> Every generated line is read, understood, owned.</li><li><strong>Treat the AI as a peer engineer you're directing</strong> — not a code vending machine. You review its output, course-correct it, and stay responsible for the architecture.</li></ul><h2>Two trends reshaping the market</h2><p>First: \"citizen developers\" are exploding. 63% of people doing vibe coding have no dev background — marketers, entrepreneurs, SMB owners. Adoption grew 520% in a year. They want custom software without going through a developer in theory. In practice, they hit walls at deployment, security, and maintenance. That's where real demand is being created.</p><p>Second: the \"SaaSpocalypse.\" In February 2026, $285 billion in SaaS market cap evaporated in a month on a simple thesis — if anyone can build custom software in minutes, why pay $50–200 per seat per month? Generic SaaS is under threat. Well-architected custom systems have never been more relevant.</p><h2>What this means for your business</h2><p>If you're using AI to build systems in your company, the question is no longer \"am I using it?\" — everyone is. The question is \"am I using it well?\"</p><p>The value is no longer in the ability to prompt — that's become table stakes. It's in the ability to <strong>hold the architecture when the AI goes sideways</strong>, catch security issues in generated code, and maintain a system that doesn't collapse six months after delivery.</p><p>That's what we build with our SMB clients. Not wild vibe coding — structured AI architecture, maintained, with systems that still work long after handover.</p><p>If you want to see what that looks like for your project, I offer a <strong><a href=\"https://calendly.com/ben40nocode/1h\">free one-hour audit</a></strong>.</p><p><em>Benjamin Bertigny — Founder NBN IA. I build AI systems for SMBs and craftsmen. Based in Bordeaux, serving all of France.</em></p>"
  },
  {
